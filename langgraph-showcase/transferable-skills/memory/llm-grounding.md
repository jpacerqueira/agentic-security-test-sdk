...
When the flag is on, `driver.py` (after `discover_scope`) calls `grounding.collect_grounding_for_run` in `asyncio.to_thread`, writes `grounding.json`, and `llm_enrich.maybe_enrich` prepends `format_grounding_prompt(orch.grounding_pack)` so LiteLlm must cite `G-00n` or say unknown.
