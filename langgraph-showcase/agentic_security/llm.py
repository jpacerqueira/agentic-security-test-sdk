"""LangChain ChatOpenAI client for an OpenAI-compatible /v1 server (Compose: llm-gateway)."""
...
def llm_stack_available() -> bool:
    """LangGraph plus ChatOpenAI. The LiteLLM proxy is a separate process."""
    try:
...
async def warm_model() -> str:
    """Load MODEL_REASONING so the first ChatOpenAI call is not a cold start. Best-effort."""
    s = get_settings()
...
async def generate_text(prompt: str, *, system: str = "", role: str = "reasoning") -> str:
    """One-shot completion through LangChain ChatOpenAI (OpenAI /v1 gateway)."""
    from langchain_core.messages import HumanMessage, SystemMessage

    s = get_settings()
    chat = s.build_chat_model(role)
    messages = []
    if system.strip():
    ...
    model_id = s.gateway_model_id(role)
    t0 = time.monotonic()
    started_at = utc_iso_ms()
    v1log.info(
