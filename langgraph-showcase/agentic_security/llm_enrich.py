"""LLM-mode enrichment via LangGraph's ChatOpenAI client (gateway OpenAI /v1)."""
...
async def run_jailbreak_probes(jail: dict) -> dict:
    ...
    jail["note"] = (
        f"Live probes executed through LangGraph ChatOpenAI against the gateway model. "
...
async def maybe_enrich(orch, kind: str, payload: dict[str, Any]) -> dict[str, Any]:
    if orch.skip_llm:
        ...
        return payload
    if not llm.llm_stack_available():
        payload["mode"] = "deterministic"
        payload["llm_error"] = "langgraph and langchain-openai are not installed"
        return payload
