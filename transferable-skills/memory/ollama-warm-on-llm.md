...
Call sites: `run_full_pipeline` when `not orch.skip_llm`; `maybe_enrich` before any generate; `POST /runs/{id}/mode` with `skip_llm=false` (`force=True`). Compose default remains `SKIP_LLM=true`; warming is per LLM run, not at container boot unless `SKIP_LLM=false`.
